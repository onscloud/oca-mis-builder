...
class MisBudgetByAccountItem(models.Model):
    ...
    _credit_or_debit_zero = models.Constraint(
        "CHECK (credit*debit=0)",
        "Wrong credit or debit value in budget item! Credit or debit should be zero.",
    )

    _credit_and_debit_positive = models.Constraint(
        "CHECK (credit+debit>=0)",
        "Wrong credit or debit value in budget item! Credit and debit should be positive.",
    )
